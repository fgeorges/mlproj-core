...
t.test('Parsing achel (base) - parameters and typical topology', ass => {
    ...
    ass.database('The content db', dbs[0], 'content', 'simple-achel-content', ['simple-achel-content-001']);
    // the app server
...
t.test('Parsing achel (prod) - inheritence and connection info', ass => {
    ...
    ass.database('The modules db', dbs[0], 'modules', 'simple-achel-modules', ['simple-achel-modules-001']);
    ass.database('The content db', dbs[1], 'content', 'simple-achel-content', ['simple-achel-content-001']);
    // the app server
    const srvs = env.servers();
    ass.equal('There must be 1 app server', srvs.length, 1);
    ass.server('The server', srvs[0], 'app', 'simple-achel', 'Default',
